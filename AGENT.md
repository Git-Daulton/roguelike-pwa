...
- A short commit/push changelog at the end
...
## Required Git Actions (must do)

For every completed task/slice:
...
- `git add -A`
- `git commit -m "<imperative message matching the slice>"`
...
If push is rejected (e.g., remote changed), stop and report the exact error output with a recommended fix (usually pull/rebase) rather than guessing.
